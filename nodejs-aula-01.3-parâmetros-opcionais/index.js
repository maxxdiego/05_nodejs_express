...
// ROTA PERFIL 
app.get("/perfil/:nome?", function (req, res) { // :nome? -> PARÂMETRO OPCIONAL
    var nome = req.params.nome // Coletando o parâmetro :nome vindo da URL (método GET) e gravando na variável nome.
    if (nome) { // Se nome existir
        res.send(`<h2>Olá, ${nome}!<br> Seja bem-vindoao seu perfil!</h2>`)
    } else {
        res.send(`<h2>Faça login para acessar o seu perfil.</h2>`)
    }
})

// ROTA VÍDEOS COM MÚLTIPLOS PARÂMETROS OPCIONAIS (:playlist e :video)
